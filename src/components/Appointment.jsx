...
const Appointment = () => {
  ...
  const fieldClasses = "w-full bg-[#16161f] border border-[#1e1e2e] rounded-xl px-4 py-3 text-white focus:border-[#2dd4bf] focus:outline-none transition-colors placeholder:text-[#4b5563]";

  return (
    <section id="appointment" className="py-24 px-4 sm:px-6 lg:px-8 bg-primary">
      <div className="max-w-7xl mx-auto grid lg:grid-cols-5 gap-12 lg:gap-20">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="space-y-6">
            {infoItems.map((item, index) => (
              <div key={index} className={`flex gap-4 items-start ${index !== infoItems.length - 1 ? 'border-b border-[#1e1e2e] pb-6' : ''}`}>
                <div className="text-[#2dd4bf] mt-1">{item.icon}</div>
                <div>
                  <div className="text-white font-semibold text-sm uppercase tracking-wider mb-1">{item.label}</div>
                  <div className="text-[#94a3b8]">{item.value}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="lg:col-span-3">
          <div className="bg-[#111118] border border-[#1e1e2e] p-8 md:p-10 rounded-3xl shadow-2xl relative overflow-hidden">
            {isSuccess ? (
              <div className="flex flex-col items-center justify-center py-12 text-center space-y-4">
                <div className="bg-[#2dd4bf]/10 p-4 rounded-full">
                  <CheckCircle className="text-[#2dd4bf]" size={48} />
                </div>
                <h3 className="text-2xl font-bold text-white">{t.appointment.success}</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      ...
                      <option value="In-Person">{t.hero.learnMore}</option>
